...
interface DragDropUploadProps {
  onUpload: (files: File[], urls: string[], uploads?: Array<{ file: File; url: string; provider?: string; fileSize?: number }>) => void;
  maxFiles?: number;
  maxSize?: number;
...
export function DragDropUpload({
  ...
  const smartUpload = async (file: File, fileIndex: number, totalFiles: number): Promise<{ file: File; url: string; provider: string }> => {
    ...
    return {
      ...
      fileSize: file.size // Add file size for storage counter
    };
  };
  ...
  const onDrop = useCallback(async (acceptedFiles: File[]) => {
    ...
    try {
      ...
      // Pass complete upload results including fileSize
      onUpload(
        uploads.map(u => u.file),
        uploads.map(u => u.url),
        uploads // Pass complete results with fileSize
      );
      ...
      if (googleDriveCount > 0 && cloudinaryCount > 0) {
        description += ` (${googleDriveCount} ، ${cloudinaryCount} )`;
  ...
  const { getRootProps, getInputProps, isDragActive, isDragReject } = useDropzone({
    ...
    onDropRejected: (rejectedFiles) => {
      rejectedFiles.forEach(rejection => {
        ...
        let errorMessage = `لا يمكن رفع ${file.name}: `;

        if (errors.some(e => e.code === 'file-invalid-type')) {
          errorMessage += 'نوع الملف غير مدعوم. الأنواع المدعومة: PDF, JPG, JPEG, PNG, GIF';
        } else if (errors.some(e => e.code === 'file-too-large')) {
          errorMessage += `الملف كبير جدًا (الحد الأقصى: ${Math.round(maxSize / (1024 * 1024))} ميجابايت)`;
        } else if (errors.some(e => e.code === 'too-many-files')) {
          errorMessage += `عدد الملفات كبير جدًا (الحد الأقصى: ${maxFiles} ملفات)`;
        } else {
    ...
    <div className="space-y-4" dir="rtl">
            ...
            <div className="mt-4 space-y-3">
              ...
              <div className="bg-gray-50 p-4 rounded-md border border-gray-200">
                ...
                <div className="flex items-center justify-between mb-3">
                  <div className="text-sm font-medium text-gray-700" data-testid="text-provider-status">
                    {uploadProvider?.replace(/[\u{1F300}-\u{1F9FF}]/gu, '').trim() || 'جاري التحديد...'}
                  </div>
                  <div className="text-sm font-bold text-blue-600" data-testid="text-speed-current">
      ...
      <div className="mt-4 p-3 bg-gray-50 rounded-lg border">
        ...
        <div className="flex items-center justify-between text-xs text-gray-500 mt-1">
          ...
          <span className={`font-medium ${currentCartSize / maxCartSize > 0.8 ? 'text-orange-600' :
