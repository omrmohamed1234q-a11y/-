...
// ===========================================
// UNIFIED CART API SYSTEM - DISCRIMINATED UNIONS
// ===========================================

// Print settings schema for print jobs
export const printSettingsSchema = z.object({
  ...
  paperType: z.enum(["plain", "glossy", "matte", "sticker", "coated"]).default("plain"),
  doubleSided: z.boolean().default(false),
});

// File metadata schema for print jobs
...
// Base cart item schema with shared fields
...
export const productCartItemSchema = baseCartItemSchema.extend({
  type: z.literal("product"),
  productId: z.string(),
  variant: z.string().optional(), // JSON string for product variants
});

// Partner product cart item schema
export const partnerProductCartItemSchema = baseCartItemSchema.extend({
  type: z.literal("partner_product"),
  productId: z.string(),
  partnerId: z.string(),
...
// Print job cart item schema
export const printJobCartItemSchema = baseCartItemSchema.extend({
  type: z.literal("print_job"),
  printSettings: printSettingsSchema,
  fileMeta: fileMetaSchema,
  pageCount: z.number().int().min(1), // Calculated or provided separately from printSettings.pages
});

// Unified cart item schema (discriminated union)
export const cartItemSchema = z.discriminatedUnion("type", [
  productCartItemSchema,
  partnerProductCartItemSchema,
  printJobCartItemSchema,
]);

// Base computed fields for all cart items
const baseComputedFields = z.object({
  id: z.string(), // Generated by server
  userId: z.string(),
  unitPrice: z.number().min(0), // Server-computed based on type
  subtotal: z.number().min(0), // Server-computed: unitPrice * quantity
  createdAt: z.date(),
});

// Cart item schemas with server-computed fields
export const productCartItemWithComputedSchema = productCartItemSchema.merge(baseComputedFields);
export const partnerProductCartItemWithComputedSchema = partnerProductCartItemSchema.merge(baseComputedFields);
export const printJobCartItemWithComputedSchema = printJobCartItemSchema.merge(baseComputedFields);

// Cart item with server-computed fields (discriminated union)
export const cartItemWithComputedSchema = z.discriminatedUnion("type", [
  productCartItemWithComputedSchema,
  partnerProductCartItemWithComputedSchema,
  printJobCartItemWithComputedSchema,
]);

// Cart response schema
export const cartSchema = z.object({
  items: z.array(cartItemWithComputedSchema),
  totalQuantity: z.number().int().min(0),
  subtotal: z.number().min(0),
  currency: z.string().default("جنيه"),
});

// ===========================================
// CART API VALIDATION SCHEMAS
// ===========================================

// Add to cart request schemas
...
export const addPartnerProductToCartSchema = z.object({
  type: z.literal("partner_product"),
  productId: z.string(),
  partnerId: z.string(),
  quantity: z.number().int().min(1).max(100).default(1),
...
export const addPrintJobToCartSchema = z.object({
  type: z.literal("print_job"),
  printSettings: printSettingsSchema,
  fileMeta: fileMetaSchema,
  quantity: z.number().int().min(1).max(10).default(1), // Lower max for print jobs
  notes: z.string().optional(),
});

// Unified add to cart request schema
export const addToCartRequestSchema = z.discriminatedUnion("type", [
  addProductToCartSchema,
  addPartnerProductToCartSchema,
  addPrintJobToCartSchema,
]);
...
// Print settings and file meta types
export type PrintSettings = z.infer<typeof printSettingsSchema>;
export type FileMeta = z.infer<typeof fileMetaSchema>;

// Cart item types (client request)
export type ProductCartItem = z.infer<typeof productCartItemSchema>;
export type PartnerProductCartItem = z.infer<typeof partnerProductCartItemSchema>;
export type PrintJobCartItem = z.infer<typeof printJobCartItemSchema>;
export type CartItem = z.infer<typeof cartItemSchema>;

// Cart item types (server response with computed fields)
export type CartItemWithComputed = z.infer<typeof cartItemWithComputedSchema>;

// Cart types
export type Cart = z.infer<typeof cartSchema>;

// API request types
export type AddProductToCartRequest = z.infer<typeof addProductToCartSchema>;
export type AddPartnerProductToCartRequest = z.infer<typeof addPartnerProductToCartSchema>;
