...
(async () => {
  ...
  server.listen({
    ...
    // User prefers manual control - no automatic cleanup

    // STARTUP CLEANUP DISABLED - Manual control only
    console.log('📋 Manual control mode: All cleanup operations require manual trigger');

    // MONITORING ONLY - No automatic cleanup
    setInterval(async () => {
      try {
        console.log('📊 Storage monitoring (manual control mode)...');
        ...
        if (storageInfo.success && !storageInfo.unlimited) {
          ...
          // Only monitoring - no automatic cleanup
          if (usagePercentage >= 90) {
            console.log('⚠️ ALERT: Storage at ' + usagePercentage.toFixed(1) + '% - manual cleanup recommended');
          } else if (usagePercentage >= 70) {
            console.log('ℹ️ INFO: Storage at ' + usagePercentage.toFixed(1) + '% - watch level');
          } else {
